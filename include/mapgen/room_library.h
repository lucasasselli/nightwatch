...
static void furnish_gallery(map_t* map, bounds_t b) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        switch (randi(4)) {
            ...
            case 3:
                // Statue
                room_add_statue(map, b, 3, y_off + 2, DIR_SOUTH);
                break;
        }
    }
...
static void furnish_corridor(map_t* map, bounds_t b) {
    ...
    // Pictures
    for (int i = 0; i < 3; i++) {
        add_picture(map, b, 0, 2 * i + 1, DIR_WEST);
        add_picture(map, b, 3, 2 * i + 1, DIR_EAST);
    }
}
...
static void furnish_square_small(map_t* map, bounds_t b) {
    ...
    switch (t) {
        ...
        case 1:
            // Sculpture
            room_add_statue(map, b, 2, 2, DIR_SOUTH);
            break;
    }
    ...
    switch (randi(2)) {
        case 0:
            ...
            switch (randi(2)) {
                case 0:
                    // Statues
                    room_add_statue(map, b, 4, 4, DIR_NORTH);
                    break;
                case 1:
                    // Benches
                    room_add_statue(map, b, 4, 4, DIR_SOUTH);
...
roomlib_room_t room_corridor_short = {
    4,
    7,
    &furnish_corridor,
    2,
...
roomlib_room_t room_corridor_long = {
    4,
    9,
    &furnish_corridor,
    2,
